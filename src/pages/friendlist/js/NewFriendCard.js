...
export default function NewFriendCard({ follow, documents }) {
  ...
  const handleOnMouseEnter = () => {
    documents[0].followers.forEach((f) => {
      if (f.uid === follow.uid) {
        f.check = true;
      }
      updateDocument(documents[0].id, {
        followers: documents[0].followers,
      });
    });
  };

  return (
    <ul className={styles.friendcard} onMouseEnter={() => handleOnMouseEnter()}>
